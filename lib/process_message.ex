defmodule ProcessMessage do
  ...
  @min_perc 0.20  # min percentage change
  #@chat_ids ['-510655586']
  ...
  defp handle_message(symbol, event_time, total_trades, price) do
    ...
    if GenServer.call(PriceGroups, :length) > 0 do
      ...
      sorted_price_groups = Enum.sort_by(list_price_groups, & &1.total_price_change) |> Enum.reverse

      any_printed = print_result(sorted_price_groups, "Top ticks:")

      any_printed = if not any_printed do
        sorted_price_groups = Enum.sort_by(list_price_groups, & &1.total_price_change) |> Enum.reverse
      ...
      any_printed = if not any_printed do
        sorted_price_groups = Enum.sort_by(list_price_groups, & &1.relative_price_change) |> Enum.reverse
  ...
  def print_result(sorted_price_groups, msg) do
    any_printed = print_result_recursive(Enum.to_list(0..@show_limit-1), sorted_price_groups, msg, false, false)
  ...
  def print_result_recursive([head | tail], sorted_price_groups, msg, any_printed, header_printed) do
    {any_printed, header_printed} = if head < length(sorted_price_groups) do
      max_price_group = Enum.at(sorted_price_groups, head)

      {any_printed, header_printed} = if not max_price_group.isPrinted do
        header_printed = if not header_printed do
          IO.puts(msg)
          header_printed = true
          header_printed
        else
